...
- DAMの採点画面内に、検出可能な35種類の歌唱技法と回数を同じ意匠のグリッドで表示
...
採点中は、DAM本体の画面内に検出可能な35種類の歌唱技法と回数をリアルタイム表示します。既存の音程バー、歌詞、4つの集計カウンターを残し、その下へ同じ暗色面・金枠の小型グリッドを配置します。まだ検出されていない技法は薄く、検出した技法と回数は明るく表示されます。

PC画面の「採点」タブでも同じ検出結果を確認できます。初期設定では未検出の技法を含めて固定配置するため、歌唱中に並び順が変わりません。

「DAM画面に表示」と「0回の技法も表示」は別々に切り替えられ、初期値はどちらもONです。回数は曲ごとに初期化され、DAM内のグリッドは一時停止中と演奏終了時に閉じ、再開時は同じ回数で戻ります。PC側には直前の結果を残しますが、採点結果はファイルへ保存しません。
